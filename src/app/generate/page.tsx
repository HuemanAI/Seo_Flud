...
// The SEOflud generator UI — reads initial prompt from ?prompt= query param
...
function GeneratorContent() {
  ...
  // Auto-generate if prompt came from homepage
  useEffect(() => {
    if (initialPrompt && initialPrompt.length >= 15) {
  ...
  async function generate(overridePrompt?: string) {
    ...
    try {
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ prompt: p, tier, email })
      })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#08090c', color: '#f2f0ff', fontFamily: "'Syne', -apple-system, sans-serif" }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 32px', borderBottom: '1px solid #1e2130', position: 'sticky', top: 0, background: 'rgba(8,9,12,0.9)', backdropFilter: 'blur(12px)', zIndex: 100 }}>
        <a href="/" style={{ fontWeight: 800, fontSize: 20, letterSpacing: '-0.03em', textDecoration: 'none', color: '#f2f0ff' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 440, textAlign: 'left' }}>
            {steps.map(step => (
              <div key={step.id} style={{
                ...
                padding: '13px 18px', borderRadius: 12, background: '#0f1117',
                border: `1px solid ${step.status === 'done' ? '#00e5a0' : step.status === 'loading' ? '#7c6dfa' : '#1e2130'}`,
                fontSize: 14, color: step.status === 'idle' ? '#5a5d72' : '#f2f0ff', transition: 'all 0.3s'
              }}>
          ...
          {activeTab === 'deploy' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                { n: '5', title: 'Submit to Google', cmd: '# Go to search.google.com/search-console\n# Submit: https://yourdomain.com/sitemap.xml', note: 'Pages start appearing in Google within 24-48 hours.' },
              ].map(step => (
          ...
          <input
            ...
            onKeyDown={e => {
              if (e.key === 'Enter') {
                ...
                generate((e.target as HTMLInputElement).value)
...
export default function GeneratePage() {
  return (
    <Suspense fallback={
      <div style={{ minHeight: '100vh', background: '#08090c', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#f2f0ff', fontFamily: 'sans-serif' }}>
        Loading...
      </div>
    }>
      <GeneratorContent />
